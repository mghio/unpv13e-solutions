                       ...
                       SA *sa, socklen_t *salenptr, struct unp_in_pktinfo *pktp)
{
    ...
    msg.msg_control = control_uns.control;
    msg.msg_controllen = sizeof(control_uns.control);
    msg.msg_flags = 0; 
#else
    bzero(s&msg, sizeof(msg));  /* make certain msg_accrightslen = 0 */
#endif
    ...
    *flagsp = msg.msg_flags;  /* pass back results */
    ...
    for (cmptr = CMSG_FIRSTHDR(&msg); cmptr != NULL; cmptr = CMSG_NXTHDR(&msg, cmptr))
    {
        ...
        {
            memcpy(&pktp->ipi_addr, CMSG_DATA(cmptrs), sizeof(struct in_addrs));
            continue;
        } 
#endif        
